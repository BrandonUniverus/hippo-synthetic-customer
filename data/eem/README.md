...
- `northlake-rates-v1.yaml` is the Phase 2 **Rate Modeler entry plan**: the rate
  ...
  tariffs stay sourced from `data/providers/`.
...
- `northlake-eem-permissions-v1.yaml` is the Phase 0 (foundation) concrete
  ...
  `data/security/northlake-eem-security-v1.yaml` against the real EEMSuite permission
...
- `northlake-company-calendar-v1.yaml` is the Phase 0 (foundation) fiscal
  ...
  fixture. Required before bills load. See `implementation/dream-customer/`.
- `northlake-instance-mapping.template.csv` is generated by
  `generators/northlake_packet.py`: one row per point, degree-day, baseline and
  aggregate record to create, with blank columns for the installed IDs.
