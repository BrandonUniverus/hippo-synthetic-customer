...
$script:IdentityScimSettings = Join-Path $script:IdentityRuntimeDirectory "scim.json"
$script:IdentityScimCredentials = Join-Path $script:IdentityRuntimeDirectory "scim-credentials.json"
$script:IdentityScimVerification = Join-Path $script:IdentityRuntimeDirectory "scim-verification.json"
$script:IdentityScimLifecycle = Join-Path $script:IdentityRuntimeDirectory "scim-lifecycle.json"
$script:IdentityEvidenceDirectory = Join-Path $script:IdentityRuntimeDirectory "evidence"
$script:IdentityCoverageCatalog = Join-Path $script:IdentityRoot "evidence\coverage-catalog.json"
$script:IdentityFaultCatalog = Join-Path $script:IdentityRoot "evidence\fault-fixtures.json"
$script:IdentityEvidenceScript = Join-Path $script:IdentityRoot "scripts\identity_evidence.py"
$script:IdentityCommandEvidence = Join-Path $script:IdentityEvidenceDirectory "command-results.json"
$script:IdentityRestartEvidence = Join-Path $script:IdentityEvidenceDirectory "restart-stability.json"
$script:IdentityEvidenceManifest = Join-Path $script:IdentityEvidenceDirectory "identity-evidence.json"
...
$script:IdentityManifest = Join-Path $script:RepositoryRoot "data\security\northlake-eem-security-v1.yaml"
