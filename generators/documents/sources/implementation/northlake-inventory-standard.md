...
standing rule set for `data/scenarios/`, `data/eem/`, the generators and the workbook;
...
- `data/scenarios/demo-university-v1.yaml` lists the physical facts: 2 sites, 6
  buildings, 21 accounts, 23 meters, 38 channels plus 11 reference channels.
- `data/eem/northlake-eem-stage1-setup-v1.yaml` and
  `data/eem/northlake-eem-metaworld-stage1b-v1.yaml` re-list buildings, meters and
...
| "Science Center" and "Library" in the scenario, providers and `implementation/synthetic-customer-v1.pdf`; "Applied Science Center" and "Main Library" everywhere EEM-facing. | Two files own the name. |
...
| A fourth company, Northlake Town Center, exists in `data/eem/northlake-town-center-v1.yaml`, the cover letter and the Tenants and Leases tab, but not in the scenario, Stage 1, the security manifest or the hierarchy. | Phase 4 content leaked into the current packet. |
...
R12. **One owner per fact.** `data/scenarios/demo-university-v1.yaml` owns the
...
company that owns each site. `data/eem/` files own only the mapping: level names,
...
1. `data/scenarios/demo-university-v1.yaml`: `companies`; `ownerCompanyId` per site;
...
2. `data/eem/northlake-eem-stage1-setup-v1.yaml`: one level list for all companies;
   `hierarchySitesFromScenario` per company; the Weather Reference site; the
   12th university group.
3. `data/eem/northlake-eem-metaworld-stage1b-v1.yaml`: mapping rules only
   ...
   buildings, meters or points; rate schedule lists match `data/providers/`.
4. `data/providers/*.yaml`: `VED-TOU-PRI`, `VED-AL-1`, `SGU-GL-1`, `RCU-IRR` and
   `RCU-FIRE`; the plant's served buildings extended to Lakeview Residence Hall
   and the Recreation and Aquatics Center.
5. `data/scenarios/northlake-onboarding-v1.yaml`: packet 0.3; support locations
...
7. `data/security/northlake-eem-security-v1.yaml` and
   `data/eem/northlake-eem-permissions-v1.yaml`: no new companies; each company's
...
8. Docs: `implementation/synthetic-customer-v1.pdf`, the intake overview and cover letter,
   the ENERGY STAR mapping, `implementation/dream-customer/phase4-tenant-rebilling-spec.pdf`,
