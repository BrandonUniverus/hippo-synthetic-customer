...
provider(
  ...
  config_ref               -- data/providers/<provider_id>.yaml
)
...
`implementation/dream-customer/`.

| Phase | New entities | Spec |
| --- | --- | --- |
| 2 Rates | `rate_determinant`, `rate_block`, `rate_effective_date`, `rate_schedule_calendar`, `rate_ratchet`, `rate_class_option`, `rate_meter_assignment` | `phase2-rate-engine-spec.pdf` |
| 3 AP/GL | `gl_account`, `gl_ud_field`, `gl_ud_value`, `ap_export_setup`, `bill_validation_rule`, `bill_validation_result`, `accrual` | `phase3-ap-gl-validation-spec.pdf` |
| 4 Tenant | `tenant_space`, `bill_allocation_config`, `bill_allocation_detail`, `bill_allocation_map`, `bill_aggregation_map`, `pt_allocation_config`, `pt_allocation_index`, `bill_gen_config` | `phase4-tenant-rebilling-spec.pdf` |
| 5 Sustainability | `emission_factor_source`, `emission_factor`, `emission_meter_map`, `emission_company_config`, `es_rating_response`, `weather_regression_result`, `benchmark_definition` | `phase5-sustainability-analytics-spec.pdf` |
| 6 Operations | `project`, `project_activity`, `project_event`, `alarm_point`, `alarm_event`, `audit_event`, `task_schedule`, `task_log_entry`, `workflow`, `mfr_saved_report`, `mfr_dashboard`, `mfr_delivery`, `mfr_delivery_recipient`, `energyai_insight`, `hmr_meter`, `hmr_read_type`, `hmr_reading`, `import_source`, `import_mapping` | `phase6-operations-engagement-spec.pdf` |
