...
are additions to `data/scenarios/demo-university-v1.yaml` (channels/points) and the
`data/eem/` setup (status history, notes, images), authored here as load-ready spec.
...
AccessLevel-3 user (per `data/eem/northlake-eem-permissions-v1.yaml`) so AP-export
...
`implementation/synthetic-source-model.pdf`). Northlake's only Digital point so far is
...
| --- | --- | --- | --- |
| Each provider's sample bills | BL | bill_image | the generated PDFs under `documents/utility-bills/<provider>/...` |
