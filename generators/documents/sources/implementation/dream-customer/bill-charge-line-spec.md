...
`data/providers/<id>.yaml` `billCharges` block — this spec maps them to the
...
`bill_charge_line.charge_type` (from `implementation/synthetic-source-model.pdf`) maps to
the rate-engine determinant math kind (see
[subsystem-gap-analysis.pdf](subsystem-gap-analysis.pdf) §3):
...
`data/providers/<id>.yaml` rate component.
