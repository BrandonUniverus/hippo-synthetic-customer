...
[customer packet](../../documents/intake-package/) and
[Student Center UI checklist](../northlake-onboarding-ui-checklist.pdf).
...
| Doc | What it is |
| --- | --- |
| `dream-customer-overview.pdf` (this document) | Goal, current state, coverage scorecard, roadmap summary, open decisions |
| [`subsystem-gap-analysis.pdf`](subsystem-gap-analysis.pdf) | Per-subsystem: what EEMSuite supports, what Northlake has, the gap, what to add |
| [`report-coverage-matrix.pdf`](report-coverage-matrix.pdf) | All 115 EEMSuite reports → data bucket → covered/partial/missing → phase |
| [`roadmap.pdf`](roadmap.pdf) | Phased plan with deliverables and acceptance criteria |
| [`bill-charge-line-spec.pdf`](bill-charge-line-spec.pdf) | Phase 1: how each provider's bills decompose into reconciling charge lines (→ EEMSuite `BillItem`) |
| [`phase1-substrate-extras.pdf`](phase1-substrate-extras.pdf) | Phase 1: bill status history, digital/status points, notes & images |
| [`phase-0-1-assertions.pdf`](phase-0-1-assertions.pdf) | Phase 0/1 known-value regression checklist (verify once loaded) |
| [`phase2-rate-engine-spec.pdf`](phase2-rate-engine-spec.pdf) | Phase 2: 10 rate schedules firing all 9 determinant kinds |
| [`phase3-ap-gl-validation-spec.pdf`](phase3-ap-gl-validation-spec.pdf) | Phase 3: GL coding, 7 AP/GL/AR export formats, validation, accruals |
| [`phase4-tenant-rebilling-spec.pdf`](phase4-tenant-rebilling-spec.pdf) | Phase 4: Town Center + sub-metered Cedar Row; every allocation/WUI/reconciliation path |
| [`phase5-sustainability-analytics-spec.pdf`](phase5-sustainability-analytics-spec.pdf) | Phase 5: GHG, ENERGY STAR ratings, weather regression, benchmarks |
| [`phase6-operations-engagement-spec.pdf`](phase6-operations-engagement-spec.pdf) | Phase 6: projects, alarms, audit, tasks/workflows, MFR delivery to 100+, EnergyAI, HMR, importers |
| [`entry-path-and-gaps.pdf`](entry-path-and-gaps.pdf) | **How each thing is created via the EEM UI** (not DB), and the register of UI gaps + decisions |
| [`product-gap-backlog.pdf`](product-gap-backlog.pdf) | EEMSuite **product features to build/connect** to close the UI gaps (cross-repo work) |
...
2. **Speak the customer's language, not EEM's.** Source data and customer-facing
   ...
   a housing community). EEM-specific jargon is confined to the `data/eem/` setup
...
4. **New themed customers are allowed** where a subsystem needs a different
   ...
   multi-company **Northlake Portfolio** is the vehicle for data/security/permission
...
6. **Created through the UI, not the database.** The end state can be stated as
   ...
   [`entry-path-and-gaps.pdf`](entry-path-and-gaps.pdf) for the per-subsystem entry
...
[`subsystem-gap-analysis.pdf`](subsystem-gap-analysis.pdf).)
...
Detail + acceptance criteria in [`roadmap.pdf`](roadmap.pdf).

> Phase specs 0–6 and their `data/eem/` UI-entry plans are authored. They still require
