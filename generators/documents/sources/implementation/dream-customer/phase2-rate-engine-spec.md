...
> **Entry path** (see [entry-path-and-gaps.pdf](entry-path-and-gaps.pdf)): nearly all
...
`implementation/synthetic-source-model.pdf` `rate_schedule` gains determinant-level
...
price channel (add to `data/scenarios/demo-university-v1.yaml` referenceChannels).
