...
> [`documents/brand/`](./) (logos/icons in `images/custom/`,
> branded email templates in `email-templates/`, filled `login-customization.pdf`,
> mockups + previews). Email tokens were verified/corrected against the real EEM
> templates. See `documents/brand/eem-portal-brand-kit.pdf` for the deployment map.
